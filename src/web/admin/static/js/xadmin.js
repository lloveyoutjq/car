;!function (win) {
  ...
  Xadmin.prototype.del_tab = function (id) {
     ...
     * @param  {[String/DOM/Array]}     urlType [打开类型]
	 * @param  {[type]}  url            [弹出层地址]
	 * @param  {[type]}  w              [宽]
	 * @param  {[type]}  h              [高]
	 * @param  {Boolean} full           [全屏]
     * @shade  {[String/Array/Boolean]} full  [遮罩]
     * @shade  {[function/Boolean]} full  [确认按钮回调函数]  如果不要按钮的话 就为false
	 * @return {[type]}        [description]
	 */
	Xadmin.prototype.open = function (title,urlType,url,w,h,full,shade,success,method) {
		if (title == null || title == '') {
	        var title=false;
        }
        ...
        if (shade) { //shade == null || shade == '' ||
            var shade= 0.4;
        }
        if(typeof(success) == "function" && typeof(method) == "function"){
            var index = layer.open({
                ...
                success:function(layero, index){
                    let html = layui.layer.getChildFrame('html', index);
                    success(layero, index, html);
                },
                btn: ['确定', '取消'],
                yes: function(index, layero){
                    let html = layui.layer.getChildFrame('html', index);
                    method(index,layero,html);
                    layer.close(index);
                }
            });
        }else if(typeof(success) == "function" && typeof(method) != "function"){
            var index = layer.open({
                type: urlType,
                area: [w+'px', h +'px'],
                fix: false, //不固定
        ...
        }else if(typeof(method) == "function" && typeof(success) != "function" ){
            var index = layer.open({
                type: urlType,
                area: [w+'px', h +'px'],
                fix: false, //不固定
